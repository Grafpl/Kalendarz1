...
namespace Kalendarz1
{
    public class WidokPanelProdukcja : Form
    {
        ...
        private void BuildUi()
        {
            ...
            // Kolumna 1: Przychody (wcześniej zamówienia)
            var bottom = new Panel { Dock = DockStyle.Fill, BackColor = Color.FromArgb(30, 30, 38) };
            var lblIn0E = new Label { Text = "Przychody (In0E)", Font = new Font("Segoe UI Semibold", 18f, FontStyle.Bold), AutoSize = true, Left = 8, Top = 6 };
            ...
            dgvIn0ESumy.Location = new Point(0, 62);
            dgvIn0ESumy.Width = bottom.Width - 8;
            dgvIn0ESumy.Height = bottom.Height - 66;
            dgvIn0ESumy.Anchor = AnchorStyles.Top | AnchorStyles.Left | AnchorStyles.Right | AnchorStyles.Bottom;
            bottom.Resize += (s, e) => { dgvIn0ESumy.Width = bottom.Width - 8; dgvIn0ESumy.Height = bottom.Height - 66; };
            bottom.Controls.Add(lblIn0E);
            bottom.Controls.Add(lblIn0ERefresh);
            bottom.Controls.Add(dgvIn0ESumy);
            ...
            dgvZamowienia.Columns.Add(new DataGridViewTextBoxColumn { Name = "Klient", DataPropertyName = "Klient", HeaderText = "Nazwa Klienta", AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill, FillWeight = 50 });
            ...
            dgvZamowienia.Columns.Add(new DataGridViewTextBoxColumn { Name = "Handlowiec", DataPropertyName = "Handlowiec", HeaderText = "Handlowiec", AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill, FillWeight = 30 });
            ...
            // Kolumna 3: Pozycje i notatki (wcześniej notatki)
            var rightPanel = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 1, RowCount = 3, BackColor = Color.FromArgb(38, 40, 50), Padding = new Padding(4) };
            rightPanel.RowStyles.Add(new RowStyle(SizeType.Percent, 55)); // Pozycje
            rightPanel.RowStyles.Add(new RowStyle(SizeType.AutoSize)); // Etykieta notatek
            rightPanel.RowStyles.Add(new RowStyle(SizeType.Percent, 45)); // Notatki

            dgvPozycje = CreateGrid(false);
            rightPanel.Controls.Add(dgvPozycje, 0, 0);

            var lblUwagi = new Label { Text = "Notatka zamówienia", Font = new Font("Segoe UI Semibold", 14.5f, FontStyle.Bold), Dock = DockStyle.Top, Padding = new Padding(4) };
        ...
        private async Task PopulateProductFilterAsync()
        {
            ...
            if (ids.Count > 0)
            {
                ...
                for (int i = 0; i < list.Count; i += batch)
                {
                    ...
                    cmd.CommandText = $"SELECT ID,kod FROM HM.TW WHERE ID IN ({string.Join(",", paramNames)})"; using var rd = await cmd.ExecuteReaderAsync(); while (await rd.ReadAsync()) { int id = rd.GetInt32(0); string kod = rd.IsDBNull(1) ? string.Empty : rd.GetString(1); _produktLookup[id] = kod; }
                }
            }
    ...
    WHERE z.DataZamowienia=@D AND ISNULL(z.Status,'Nowe') NOT IN ('Anulowane')";
            ...
            dgvZamowienia.DataSource = orderListForGrid.OrderBy(o => o.Handlowiec).ThenBy(o => o.Klient).ToList();
        ...
        private async Task LoadPozycjeForSelectedAsync()
        {
            ...
            dt.Columns.Add("Notatka", typeof(string));
            ...
            { 
                ...
                var (c, p) = CalcContainersAndPallets(kod, ord.Ilosc); 
                dt.Rows.Add(kod, ord.Ilosc, wyd, ord.Ilosc - wyd, c, p, ord.Notatka); 
            }
...
+ (_filteredProductId.HasValue ? " AND MZ.idtw=@P" : "") + " GROUP BY MZ.idtw";
        ...
        private async Task LoadIn0ESummaryAsync()
        {
            ...
            lblIn0ERefresh.Text = "Ostatnie odświeżenie: " + DateTime.Now.ToString("HH:mm:ss");
        }
