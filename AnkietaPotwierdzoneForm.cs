...
namespace Kalendarz1
{
    ...
    public partial class AnkietaPotwierdzoneForm : Form
    {
        ...
        private void BuildUi()
        {
            ...
            var top = new Panel { Dock = DockStyle.Top, Height = 34, BackColor = Color.FromArgb(245, 247, 250) };
            var lblDate = new Label { Left = 12, Top = 9, AutoSize = true, Text = "Dzień:", Font = new Font("Segoe UI", 10f, FontStyle.Regular) };
            _dtp = new DateTimePicker { Left = 62, Top = 5, Width = 140, Format = DateTimePickerFormat.Short };
            _dtp.ValueChanged += OnDateChanged;
            _lblInfo = new Label { Left = 270, Top = 9, AutoSize = true, ForeColor = Color.FromArgb(80, 90, 100), Text = "Wczytywanie…" };
            top.Controls.Add(lblDate);
            top.Controls.Add(_dtp);
            top.Controls.Add(_lblInfo);
            Controls.Add(top);

            _scrollHost = new Panel { Dock = DockStyle.Fill, AutoScroll = true, Padding = new Padding(10), BackColor = Color.White };
            _flow = new FlowLayoutPanel { Dock = DockStyle.Fill, AutoScroll = false, WrapContents = true, FlowDirection = FlowDirection.TopDown, Padding = new Padding(0, 20, 0, 0), Margin = new Padding(0) };
        ...
        private async Task ReloadForDateAsync(DateTime day)
        {
            await Task.Run(() => { _deliveries = LoadDeliveriesForDay(day); });
            ...
            foreach (var d in _deliveries.OrderByDescending(x => x.WagaDek).ThenByDescending(x => x.Auta).ThenBy(x => x.Dostawca))
            {
        ...
        private Control BuildCard(DeliverySurveyItem d)
        {
            ...
            var lblDostawca = new Label { Left = 8, Top = 20, AutoSize = true, Font = new Font("Segoe UI Semibold", 10f), Text = d.Dostawca };
            ...
            var lblDetails = new Label { Left = lblDostawca.Right + 8, AutoSize = true, Font = new Font("Segoe UI", 8.5f), ForeColor = Color.FromArgb(64, 64, 64), Text = detailsText };
            ...
            var grpStanPtakow = MakeGradeGroup("Stan ptaków", 236); // NOWA KATEGORIA

            // DODANE: Rysowanie czerwonej ramki wokół GroupBox 'Stan ptaków'
            grpStanPtakow.Host.Paint += (sender, e) =>
            {
                if (sender is GroupBox box)
                {
                    // Ustawienie pióra na czerwony kolor o grubości 2 pikseli
                    using (Pen pen = new Pen(Color.Red, 2))
                    {
                        // Rysowanie prostokąta, który jest prostokątem klienta GroupBox'a
                        // Musimy przesunąć prostokąt o 1 piksel w każdym kierunku, aby ramka była widoczna
                        // i nie kolidowała z wewnętrznymi elementami GroupBox'a.
                        // I odjąć 1 piksel od szerokości/wysokości, aby zmieściła się w granicach kontrolki.
                        e.Graphics.DrawRectangle(pen, 0, 0, box.Width - 1, box.Height - 1);

                        // Opcjonalnie: aby ramka nie nachodziła na tekst nagłówka GroupBox'a
                        // można by rysować 4 linie oddzielnie i pomijać obszar tekstu.
                        // Ale dla prostoty na razie rysujemy pełny prostokąt.
                    }
                }
            };
            ...
            btnSave.Click += (s, e) =>
            {
                try
                {
                    ...
                    int? ocStanPtakow = grpStanPtakow.Picker.GetScoreOrNull();

                    UpsertFeedback(
                        d.Lp, _day, _userId,
                        ocCena, ocTrans, ocKom, ocElas, ocStanPtakow,
            ...
            grpStanPtakow.Picker.SetUnknownDefault();

            TryInitFromDb(d.Lp, _userId, grpCena.Picker, grpTransport.Picker,
                grpKomunikacja.Picker, grpElastycznosc.Picker, grpStanPtakow.Picker,
                txtNote, card, btnSave);
            ...
            card.Controls.Add(grpStanPtakow.Host);
        ...
        {
            var gb = new GroupBox { Left = 8, Top = top, Width = 420, Height = 44, Text = title, RightToLeft = RightToLeft.Yes };
            var picker = new GradePicker(gb);
            return (gb, picker);
        }

        private void UpsertFeedback(
            int dostawaLp, DateTime dataDostawy, string userId,
            int? ocCena, int? ocTransport, int? ocKomunikacja, int? ocElastycznosc,
            int? ocStanPtakow, // NOWA KATEGORIA
            string? notatka)
        {
            const string sql = @"
...
BEGIN
    ...
    SET DataDostawy       = @Data,
        ...
        OcenaStanPtakow   = @OcStanPtakow, -- NOWA KATEGORIA
        Notatka           = @Notatka
...
BEGIN
    INSERT INTO dbo.DostawaFeedback
        (DostawaLp, DataDostawy, DataAnkiety, Kto,
         OcenaCena, OcenaTransport, OcenaKomunikacja, OcenaElastycznosc, OcenaStanPtakow, Notatka) -- NOWA KATEGORIA
    VALUES
        (@Lp, @Data, @now, @Kto,
         @OcCena, @OcTrans, @OcKom, @OcElas, @OcStanPtakow, @Notatka); -- NOWA KATEGORIA
END";
            ...
            cmd.Parameters.Add("@OcStanPtakow", SqlDbType.TinyInt).Value = (object?)ocStanPtakow ?? DBNull.Value; // NOWA KATEGORIA
...
ORDER BY WagaDek DESC, Auta DESC, Dostawca ASC;", conn);
        ...
        private Task<DateTime?> FindFirstFutureDayWithConfirmedDeliveriesAsync(DateTime startExclusive)
        {
        ...
        private bool HasConfirmedDeliveriesForDay(DateTime day)
        {
            ...
            conn.Open();
            using var cmd = new SqlCommand(@"
SELECT TOP 1 1 FROM dbo.HarmonogramDostaw h
WHERE CAST(h.DataOdbioru AS date) = @d AND h.Bufor = 'Potwierdzony';", conn);
            cmd.Parameters.Add("@d", SqlDbType.Date).Value = day.Date;
            cmd.Parameters.Add("@Kto", SqlDbType.NVarChar, 64).Value = _userId;
        ...
        private void TryInitFromDb(
            ...
            GradePicker stanPtakow, // NOWA KATEGORIA
            TextBox note, Panel card, Button saveBtn)
        {
            const string sql = @"
SELECT TOP 1 OcenaCena, OcenaTransport, OcenaKomunikacja, OcenaElastycznosc, OcenaStanPtakow, Notatka -- NOWA KATEGORIA
...
WHERE DostawaLp=@Lp AND Kto=@Kto;";
            ...
            stanPtakow.SetScoreOrUnknown(s(r["OcenaStanPtakow"])); // NOWA KATEGORIA
        ...
        private sealed class GradePicker
        {
            ...
            {
                if (score is null) { SetUnknownDefault(); return; }
                _rbNieWiem.Checked = false;
                _rbKontrakt.Checked = false;
                foreach (var rb in _scoreRbs) rb.Checked = (int)rb.Tag == score.Value;
            }
