#nullable enable // Włączamy sprawdzanie null, aby kod był czystszy
...
namespace Kalendarz1
{
    public partial class AdminChangeRequestsForm : Form
    {
        ...
        public AdminChangeRequestsForm(string connString, string appUser)
        {
            ...
            InitializeComponent(); // Ta metoda jest teraz w pliku .Designer.cs
            HookEvents();
            ZaladujNaglowki();
            WyswietlDaneZaznaczenia(); // Ustaw stan początkowy kontrolek
        }
...
SELECT
  ...
  cr.UzasadnienieDecyzji           AS [Notatka administratora]
...
");
            ...
            if (zaznaczoneId.Any())
            {
                ...
                {
                    if (row.DataBoundItem is DataRowView rv && idSet.Contains(Convert.ToInt64(rv["ID Wniosku"])))
                    {
                        row.Selected = true;
                    }
                }
        ...
        {
            if (dgvNaglowki.SelectedRows.Count == 1 && dgvNaglowki.CurrentRow?.DataBoundItem is DataRowView rv)
            {
                panelAkcji.Enabled = true;
            ...
            {
                panelAkcji.Enabled = dgvNaglowki.SelectedRows.Count > 0;
                txtNotatka.Enabled = false;
                btnZapiszNotatke.Enabled = false;
...
SELECT i.Field AS [Pole], i.OldValue AS [Stara wartość], i.ProposedNewValue AS [Nowa wartość]
FROM dbo.DostawcyCRItem i WHERE i.CRID = @id ORDER BY i.ItemID;", conn);
        ...
        // ===== AKCJE =====
        private List<long> WybraneCrid() => dgvNaglowki.SelectedRows.Cast<DataGridViewRow>()
...
UPDATE dbo.DostawcyCR SET Status = N'Zdecydowany', DecyzjaTyp = N'Zaakceptowano', DecyzjaKto = @u, DecyzjaKiedyUTC = SYSUTCDATETIME()
WHERE CRID=@id AND Status='Proposed';", conn))
        ...
        private void OdrzucZaznaczone()
        {
            ...
            foreach (var crid in list)
            {
                ...
                using var cmd = new SqlCommand(@"
UPDATE dbo.DostawcyCR SET Status = N'Zdecydowany', DecyzjaTyp = N'Odrzucono', DecyzjaKto = @u, DecyzjaKiedyUTC = SYSUTCDATETIME()
WHERE CRID=@id AND Status='Proposed';", conn);
                cmd.Parameters.AddWithValue("@u", _appUser);
                cmd.Parameters.Add("@id", SqlDbType.BigInt).Value = crid;
                ok += cmd.ExecuteNonQuery();
        ...
        private bool ZastosujWniosek(long crid, SqlConnection conn, out string msg)
        {
            ...
            using (var cmd = new SqlCommand("SELECT DostawcaID, Status, DecyzjaTyp, EffectiveFrom FROM dbo.DostawcyCR WHERE CRID=@id;", conn, trx))
            {
                ...
                id = rd["DostawcaID"] as string; status = rd["Status"] as string;
                decyzjaTyp = rd["DecyzjaTyp"] as string; eff = rd["EffectiveFrom"] as DateTime?;
            }

            if (id == null || !status!.Equals("Zdecydowany", StringComparison.OrdinalIgnoreCase) || !decyzjaTyp!.Equals("Zaakceptowano", StringComparison.OrdinalIgnoreCase))
            ...
            {
                da.SelectCommand.Transaction = trx; da.SelectCommand.Parameters.Add("@id", SqlDbType.BigInt).Value = crid; da.Fill(items);
            }
            ...
            {
                setCtx.Parameters.AddWithValue("@k1", "AppUserID"); setCtx.Parameters.AddWithValue("@v1", _appUser);
                setCtx.Parameters.AddWithValue("@k2", "ChangeReason"); setCtx.Parameters.AddWithValue("@v2", $"CR {crid}: akceptacja=zastosowanie");
            ...
            foreach (DataRow r in items.Rows)
            {
                ...
                if (!AllowedColumns.TryGetValue(field, out var kind)) { msg = $"Nieobsługiwane pole: {field}"; trx.Rollback(); return false; }
                ...
                if (!TryBindValueParameter(up, "@val", kind, newVal, out msg)) { trx.Rollback(); return false; }
                up.ExecuteNonQuery();
            }
        ...
        private string? PobierzStatus(long crid, SqlConnection conn)
        {
            ...
            c.Parameters.Add("@id", SqlDbType.BigInt).Value = crid; return c.ExecuteScalar() as string;
        }
        private DateTime? PobierzEffectiveFrom(long crid, SqlConnection conn)
        {
            ...
            c.Parameters.Add("@id", SqlDbType.BigInt).Value = crid; return c.ExecuteScalar() as DateTime?;
        }
